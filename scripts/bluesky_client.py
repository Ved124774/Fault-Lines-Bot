...
bluesky_client.py
-----------------
All Bluesky API interactions in one place: login, posting, replying,
...
class BlueskyClient:
    ...
    def get_trending_posts(self, limit: int = 20):
        queries = [
    ...
    def find_and_follow_relevant_accounts(self, posts, max_follows: int = 5) -> int:
        followed = 0
        seen_dids = set()
    ...
    def format_posts_for_prompt(self, posts) -> str:
        lines = []
        for p in posts[:15]:
            ...
            if text:
                lines.append(f"@{author} ({likes} likes): {text[:200]}")
        return "\n".join(lines)
